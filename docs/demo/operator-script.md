...
| 2 | Run the suites | `make lint && make test` | Control plane, SDK, agents, contract, and integration tests green (starts PostgreSQL) |
| 3 | Start the agent stack | `make fake-up` | `fake-platform` (127.0.0.1:8090) and `registry` (127.0.0.1:5001) healthy |
...
| 10 | Check the results sheet | `curl -s localhost:8090/fake/v1/state/sheets \| python -m json.tool` | `Results` rows at the source row numbers with masked numbers, never full ones |
| 11 | Failure injection | `curl -s -XPOST localhost:8090/fake/v1/faults -H 'content-type: application/json' -d '{"target":"broker.sheets.update","mode":"error","count":6,"status":503}'`, then repeat step 9 | Warnings are logged, rows are still written, and each number is called exactly once (`/fake/v1/state/calls`) |
