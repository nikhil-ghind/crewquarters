...
| `ctx.llm.chat(profile, messages, *, temperature, max_output_tokens, response_schema, response_model, idempotency_key)` | Returns `ChatResult(text, structured, parsed, usage, finish_reason, provider, model, locality, latency_ms, request_id)`. A family (`local.general`) resolves to the single granted variant. Cloud profiles (`openai.*`, `anthropic.*`) must be named explicitly and are never a fallback. Tools are not supported in v1alpha1. |
