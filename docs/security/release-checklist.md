...
| 6 | Container access attempts to the host, database, model endpoint, metadata/host gateway and internet fail | The contract-probe agent runs in the hardened container on the internal network and checks: it is not root, the root filesystem is read-only, and it cannot connect to 1.1.1.1:443, `host.docker.internal:80` or example.com:443. `check_image.py` verifies non-root `Config.User` and the setuid allowlist for every image. | CI `agent-e2e` (`tests/e2e/test_e2e_agents.py::test_contract_probe_passes_every_check_in_the_hardened_container`, `test_images_run_as_non_root_and_self_check`); CI `image-security`; CI `realstack-e2e` (`tests/realstack/test_contract_probe.py::test_capability_matrix_and_isolation_from_inside_the_agent_container`: from a real agent container started by the real runtime daemon, PostgreSQL, the control API, the gateway, knowledge, the proxy, the host bridge and the internet are unreachable, DNS does not resolve, and there is no Docker socket) | **Done (laptop)**. Not yet probed: the cloud metadata address `169.254.169.254` and a real vLLM endpoint on `cq-models`; the GB10 run of the same suite is pending hardware (owners: Person 2, Person 5). |
...
decision. The project owner accepted every proposed decision on 2026-09-25.

| Item | License | Decision (accepted 2026-09-25) |
| --- | --- | --- |
...
| NVIDIA vLLM container | NVIDIA Deep Learning Container License | Pulled from NGC on the appliance (the default). Not redistributed: offline bundles for others are built without `--with-vllm` until its license is reviewed for redistribution. |
| Xenova/jina-embeddings-v2-small-en | No license in the repository; the base model is Apache-2.0 | Accept with attribution to Jina AI in NOTICE. |
...
| --- | --- | --- |
| Isolation is not yet probed against the cloud metadata address or a real vLLM endpoint (check 6; the database, Docker socket, services and internet are covered by `realstack-e2e`) | Person 2, Person 5 | No |
