...
| `CQ_PUBLIC_BASE_URL` | URL | `http://localhost:8080` | no | demo-cpu, dgx (LAN HTTPS: `https://NAME.local`) | The origin the owner's browser uses. The broker builds the Google redirect URI (`…/api/v1/connections/google/callback`) and the post-OAuth return from it; Twilio callbacks use it too unless `CQ_TWILIO_CALLBACK_BASE_URL` is set. The control API reports it read-only as `callbackBaseUrl` and `callbackUrls.googleRedirectUri` in `GET /api/v1/settings` (ADR 0009). Set the same value on the control API and the broker |
| `CQ_TWILIO_CALLBACK_BASE_URL` | URL | unset (empty = `CQ_PUBLIC_BASE_URL`) | no | demo-cpu, dgx (callback tunnel) | Public HTTPS origin for Twilio voice/gather/status callbacks, usually the tunnel hostname. The broker builds the Twilio URLs from it **and validates Twilio signatures against it**; the control API reports it as `callbackUrls.twilioCallbackBase`. Not a browser origin (never added to the allowed origins). Set the same value on the control API and the broker (ADR 0009, "Revision") |
...
| `CQ_CHAT_MIN_RELEVANCE` | float (0–1) | `0.3` | no | — | Control API: in `when_relevant` chat, a retrieved passage is used only when its cosine similarity reaches this cutoff. Applies to every embedding profile without an entry in `CQ_CHAT_MIN_RELEVANCE_BY_PROFILE` |
| `CQ_CHAT_MIN_RELEVANCE_BY_PROFILE` | JSON object (profile → float) | `{"fake.hashing-512": 0.1}` | no | — | Per-embedding-profile cutoffs. The knowledge service reports `embeddingProfile` with every query and the control API picks the cutoff for it. The fake hashing embedder (`CQ_EMBEDDING_MODE=fake`) measures word overlap and scores relevant passages around 0.1–0.3, so its cutoff is lower. Setting this variable replaces the whole map |
...
| `CREWQUARTERS_URL`, `CREWQUARTERS_WAIT_SECONDS` | URL, int | `ui-url`, `180` | no | desktop launcher | Override the launcher's address and readiness wait |
