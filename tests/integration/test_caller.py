"""Caller end to end: consent rules, approval, idempotent calls, and Sheets results."""
...
def run_caller(
    ...
    **kw: Any,
    ...
    """Load the scenario, then register the answer (loading a scenario replaces auto-answers)."""
...
def assert_no_full_numbers(client: FakePlatformClient, outcome: RunOutcome, sheet: str) -> None:
    ...
    assert not FULL_NUMBER.search(json.dumps(results_tab(client, sheet))), (
        "full number in the results sheet"
    )
    assert not FULL_NUMBER.search(json.dumps(client.state("inputs"))), (
        "full number in the approval request"
    )
...
def test_approved_run_calls_only_eligible_rows_and_writes_results(
    ...
    assert (by_row[2]["callStatus"], by_row[2]["transcript"]) == (
        "answered_speech",
        "Yes, I'll be there",
    )
...
def test_approval_request_previews_masked_recipients_and_exact_count(
    ...
    outcome = launch(
        fake_client, "caller", manifest, installation, tmp_path, on_launch=answer_after_review
    )
    assert outcome.state == "SUCCEEDED", outcome.log
    assert captured["title"] == "Approve 3 automated calls"
    assert captured["key"].startswith("confirm-calls-v1:")
    card = captured["preview"]
    assert [c["label"] for c in card["choices"]] == ["Approve 3 calls", "Cancel run"]
    recipients = card["blocks"][0]
    assert [row[2] for row in recipients["rows"]] == ["••••0101", "••••0103", "••••0105"]
    assert "3 automated calls will be placed now" in card["consequence"]
...
def test_failed_sheet_writes_are_retried_without_redialing(
    ...
    fake_client.add_fault(
        "broker.sheets.update", "error", count=6, status=503, code="PROVIDER_UNAVAILABLE"
    )
    outcome = run_caller(fake_client, tmp_path, "caller-basic", BASIC, APPROVE)
    assert outcome.state == "SUCCEEDED", outcome.log
    assert all(
        r["sheetWrite"] == "written" for r in outcome.result["rows"] if r["consent"] == "validated"
    )
    assert set(fake_client.state("calls")["byNumber"].values()) == {1}
    assert any("sheet write" in e["payload"]["message"] for e in outcome.events_of("run.log"))
...
def test_interrupted_run_retries_without_redialing(
    fake_client: FakePlatformClient, tmp_path: Path
) -> None:
    manifest, installation = prepare(fake_client, "caller", "caller-basic", BASIC)
    ...
    def kill_after_first_row(handle: LaunchHandle, run_id: str) -> None:
        fake_client.wait_for(
            lambda: len(results_tab(fake_client)) >= 2 and bool(results_tab(fake_client)[1]),
            timeout=30,
        )
        handle.kill()

    first = launch(
        fake_client, "caller", manifest, installation, tmp_path, on_launch=kill_after_first_row
    )
    ...
    requested = [e for e in second.events if e["type"] == "run.input_requested"]
...
def test_no_eligible_rows_needs_no_approval(
    fake_client: FakePlatformClient, tmp_path: Path
) -> None:
    ...
    assert outcome.events_of("run.input_requested") == []
