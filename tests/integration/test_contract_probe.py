"""contract-probe exercises the broker contract end to end against the fake platform."""
...
def test_all_contract_checks_pass_against_the_fake(tmp_path: Path) -> None:
    ...
    assert len(outcome.events_of("run.log")) >= 60
    # Denials, LLM usage, and connector calls are audit records, never run events.
    assert all(e["type"].startswith("run.") for e in outcome.events)


def test_denied_access_is_audited(fake_client: FakePlatformClient, tmp_path: Path) -> None:
    manifest, installation = install(
        fake_client, {"checks": ["permissions"], "expectIsolation": False}
    )
    ...
    assert outcome.state == "SUCCEEDED", outcome.log
    [denied] = [a for a in fake_client.state("audit") if a["action"] == "capability.denied"]
    assert (denied["capability"], denied["operation"]) == (
...
def install(client: FakePlatformClient, config: dict[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    installation = client.install(
        "contract-probe", manifest["metadata"]["version"], config, manifest["spec"]["permissions"]
    )
    return manifest, installation["id"]


def test_cancellation_check_ends_the_run_cancelled(
    fake_client: FakePlatformClient, tmp_path: Path
) -> None:
    manifest, installation = install(
        fake_client, {"checks": ["cancellation"], "expectIsolation": False}
    )
    launcher = ProcessLauncher(manifest["spec"]["entrypoint"], agent_dir=PROBE, log_dir=tmp_path)
    ...
    def cancel_when_waiting(handle: LaunchHandle, run_id: str) -> None:
        def waiting() -> bool:
            return any(
                e["type"] == "run.log" and "waiting for cancellation" in e["payload"]["message"]
    ...
    outcome = run_agent(
        fake_client, launcher, installation, on_launch=cancel_when_waiting, timeout=30
    )
    assert outcome.state == "CANCELLED", outcome.log
    assert [e["payload"]["to"] for e in outcome.events_of("run.state_changed")][-2:] == [
        "CANCELLING",
        "CANCELLED",
    ]
...
def test_scheduled_run_reports_scheduled_for(
    fake_client: FakePlatformClient, tmp_path: Path
) -> None:
    manifest, installation = install(
        fake_client, {"checks": ["handshake"], "expectIsolation": False}
    )
    launcher = ProcessLauncher(manifest["spec"]["entrypoint"], agent_dir=PROBE, log_dir=tmp_path)
    outcome = run_agent(
        fake_client,
